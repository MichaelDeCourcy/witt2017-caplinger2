...
```{r plotlog1, fig.align='center', fig.width=9, fig.height=4, echo=FALSE}
p1 <- default %>%
...
```{r plotlog2, fig.align='center', fig.width=6, fig.height=4}
default %>%
  mutate(prob = ifelse(default == "Yes", 1, 0)) %>%
  ggplot(aes(balance, prob)) +
  geom_point(alpha = .15) +
...
```{r plotlog3, fig.align='center', fig.width=11, fig.height=6, echo=FALSE}
avg <- train %>% 
  mutate(prob = ifelse(default == "Yes", 1, 0)) %>% 
  group_by(student) %>% 
...
```{r plotlog4, fig.align='center', fig.width=6, fig.height=4}
...
```{r plotlog5, fig.align='center', fig.width=6, fig.height=4}
...
[^islr_log]: This tutorial was built as a supplement to chapter 4, section 3 of [An Introduction to Statistical Learning](http://www-bcf.usc.edu/~gareth/ISL/)
